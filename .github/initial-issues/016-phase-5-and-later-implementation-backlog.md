...
以下はAtCoderアクセスを伴うため、すべて `agent: needs-review-carefully` を追加する。

* Milestone: `v0.1.0-alpha.3: sample downloader`

| Issue candidate | Labels | Branch suffix | Commit |
| --- | --- | --- | --- |
| Add rate-limited AtCoder HTTP client foundation | `type: feature`, `area: core`, `agent: needs-review-carefully` | `atcoder-http-client` | `feat(core): add AtCoder HTTP client` |
| Fetch contest task list and task pages | `type: feature`, `area: parser`, `agent: needs-review-carefully` | `fetch-atcoder-tasks` | `feat(core): fetch AtCoder task pages` |
| Parse Japanese and English samples from fixtures | `type: feature`, `area: parser`, `agent: codex-ready` | `sample-parser` | `feat(parser): parse sample testcases` |
| Save sanitized debug HTML on parser failure | `type: feature`, `area: parser`, `agent: needs-review-carefully` | `parser-debug-html` | `feat(parser): save sanitized debug HTML` |
| Implement `cargo ac download` | `type: feature`, `area: cli`, `agent: needs-review-carefully` | `download-command` | `feat(cli): download contest samples` |

Parser fixtureだけを扱うIssueはネットワーク非依存とし、HTTP取得Issueから分離する。
